...
let ticketPriceTotal = [runners.map(runner => runner.donation).reduce((pVal,iniVal) => pVal + iniVal)];
...
// Problem 2
// After checking again, the large and 2XL shirts won't be available for the event due to an ordering issue.  Get a list of the emails of runners with missing shirts to contact them about this issue.
let noShirts = runners.filter(runner => runner.shirt_size === 'L' || runner.shirt_size === '2XL').map(runner => runner.email);

console.log(noShirts);

// Problem 3
// The event director wants to send special, personalized thank you letters to the top 10 donations. Create an array that contains the details of these businesses.
top10Donors = runners.filter(runner => runner.donation >= (runners.map(runner => runner.donation).sort(function(a, b){return b - a})[9]));

console.log(top10Donors);